...
async def get_lesson_analytics(
    ...
    return [AnalyticsInDB(**a) for a in analytics]
...
async def get_lesson_progress(
    ...
    # Tüm atanmış dersleri bul
    assigned = await db.assignedLessons.find({
        "lessonID": lesson_id
    }).to_list(length=None)
    
    total = len(assigned)
    completed = sum(1 for a in assigned if a["status"] == "Completed")
    in_progress = sum(1 for a in assigned if a["status"] == "In Progress")
    not_started = sum(1 for a in assigned if a["status"] == "Assigned")
